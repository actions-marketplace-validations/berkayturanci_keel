...
allowed-tools: Bash(git:*), Bash(gh:*), Bash(./gradlew:*), Bash(command:*), Bash(node:*), Bash(cd:*), Bash(npm:*), Read, Edit, mcp__github__create_pull_request
argument-hint: [optional PR title override]
---

You are wrapping up the current work session for SmartInventory.
...
- Output `mcp` ⇒ `GH_MODE=mcp`. Step 4 substitutes `mcp__github__create_pull_request` (owner=`berkayturanci`, repo=`smartinventory`, base=current PR target, head=current branch, title=resolved title, body=the same body string). The PR is opened ready (not draft) in both modes.
...
## Step 1 — Sanity check
1. !`git status --short`
2. If on `main` or `develop`, ABORT — tell the user to switch to a feature branch.
...
- !`cd android && ./gradlew test lint --no-daemon 2>&1 | tail -50`
- If Realm model files changed: !`cd android && ./gradlew test --tests "*Migration*" --no-daemon 2>&1 | tail -30`
- If billing files changed: !`cd android && ./gradlew test --tests "*Billing*" --no-daemon 2>&1 | tail -30`
- If Web config files changed: !`node -e "JSON.parse(require('fs').readFileSync('web/firebase.json')); JSON.parse(require('fs').readFileSync('web/database.rules.json')); JSON.parse(require('fs').readFileSync('web/functions/package.json')); JSON.parse(require('fs').readFileSync('web/functions/package-lock.json'))"`
- If Web Functions dependencies changed: !`cd web/functions && npm ci`
